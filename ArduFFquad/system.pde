...
static void startup_ground(bool force_gyro_cal)
{
    ...
    if (airspeed.enabled()) { // from here (05/08/2014-Menno)
...
#define CRUISE_PITCH_BAND_BEGIN -3000 // (02/03/2014-Menno) // in centidegrees
...
{       
    ...
    switch(mode) {
        ...
        case CRUISE:      // (26/02/2014-Menno)
            ...
            if (g.quaternion_alt_hold_type==0){
              set_throttle_mode(THROTTLE_HOLD);}
            else {set_throttle_mode(THROTTLE_CRUISE);}  
            ...
            counter_failsafe = 0; // safeguard to prevent pitch from being beyond -90degrees for to long
            break;
...
static bool should_log(uint32_t mask){ // (01/08/2014-Menno)
