...
#else
    g.rc_3.set_default_dead_zone(g.thr_in_deadband);
    g.rc_4.set_default_dead_zone(40);
#endif
    g.rc_6.set_default_dead_zone(0);
}

static void init_rc_in() // called when init_ardupilot is called in ArduCopter.pde
{
    // set rc channel ranges
    g.rc_1.set_angle(g.roll_input_max);    // sets maximal stick deviation to correspond to ROLL_INPUT_MAX centidegrees // (23/03/2014-Menno)
    g.rc_2.set_angle(g.pitch_input_max);    // sets maximal stick deviation to correspond to PITCH_INPUT_MAX centidegrees // (23/03/2014-Menno)
...
static void read_radio()
{
    ...
    if (hal.rcin->valid_channels() > 0) {
        ...
        g.rc_1.set_pwm(periods[rcmap.roll()-1]);    // creates rc_1.control_in with max stick deviation equal to control_in = PITCH_ROLL_ANGLE_MAX
        g.rc_2.set_pwm(periods[rcmap.pitch()-1]);   // creates rc_2.control_in with max stick deviation equal to control_in = PITCH_ROLL_ANGLE_MAX
